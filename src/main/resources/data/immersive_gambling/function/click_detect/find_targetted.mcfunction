...
execute on target store result score #bool ig.bool if entity @s[tag=clicked]
...
execute if entity @s[tag=ig.bj.bet] if score #bool ig.bool matches 1 at @s as @n[tag=clicked] at @s run function immersive_gambling:blackjack/click/bet {mode: "place"}
